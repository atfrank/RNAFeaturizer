...
void usage(){
  std::cerr << "====================================================" << std::endl;
  std::cerr << "Compute Cavity Fingerprint" << std::endl;
  std::cerr << "Input: .pdb file containing RNA structure (and cavity information). Optional: .mol2 file containing cavity information (if not in .pdb file)." << std::endl;
  std::cerr << "Output: Cavity Fingerprints as semi-structured data (similar to json). Could be directly read into Python dictionary." << std::endl;
  std::cerr << "====================================================" << std::endl;
  exit(0);
}
...
int main (int argc, char **argv){
  ...
  // get fingerprints
  vector<Atom*> atoms;
  if (mol2file.length() != 0)
    atoms = FileProcess::loadMol2(mol2file)->getAtmVec();
  else
    atoms = Select::makeSelVec(mol, ":UNK.");
  MFP::MolDev dists;
